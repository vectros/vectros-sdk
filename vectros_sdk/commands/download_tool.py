"""
Command-line tool and API for downloading tools from the AIOS Tool Hub.

Provides `download_tool` programmatic function and `main` CLI entry point.
"""
...
def download_tool(
    ...
    """
    Download a tool package from the AIOS Tool Hub to local storage.

    Args:
        tool_author (str): Author identifier for the tool.
        tool_name (str): Name of the tool to download.
        tool_version (Optional[str], optional): Tool version string. Defaults to None.
        toolhub_url (str, optional): Tool Hub endpoint URL. Defaults to DEFAULT_TOOLHUB_URL.
        target_dir (Optional[str], optional): Destination directory on local machine. Defaults to core tool path.
        timeout (int, optional): Request timeout in seconds. Defaults to 30.

    Returns:
        Dict[str, Any]: Status dictionary containing success flag, tool name, author, and target path.

    Example:
        >>> from vectros_sdk.commands.download_tool import download_tool
        >>> res = download_tool("demo_author", "demo_tool")
...
def main(args: Optional[List[str]] = None) -> int:
    """
    CLI entry point for `download-tool`.
